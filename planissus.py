...
class Animal():
  ...
  def generateOffspringProperties(self):
    socialAttitude = np.random.normal(self.socialAttitude, 20)
...
class Erbast(Animal):
  ...
  def generateOffspring(self, idPride, livingSpecies):
    ...
    successors = math.ceil(2*(1-sizePride)) if livingSpecies > 20 else 2
...
class Carviz(Animal):
  ...
  def generateOffspring(self, idHerd, livingSpecies):
    ...
    successors = math.ceil(2*(1-sizeHerd)) if livingSpecies > 20 else 2
    for successor in range(0,successors):
      energy, socialAttitude, lifetime = self.generateOffspringProperties()
...
def relativeMovement(rel):
  ...
  if(randomNumber <= 0.3):
    addPos = 0
  elif(randomNumber > 0.3 and randomNumber < 0.3+rel):
    addPos = 1
  else:
    addPos = -1
  return addPos
...
class Herd(Group):
  ...
  def groupInteractions(self):
    for member in self.memberList:
      threshold = np.random.normal(40,10)
  ...
  for row, col in np.ndindex(cells.shape):
    if cells[row][col]['type']=='ground':
      ...
      if numHerds > 1:
        ...
        threshold = np.random.normal(5, 5)
    ...
    for event in pygame.event.get():
      ...
      if pygame.mouse.get_pressed()[0]:
        ...
        if cell.get('type') == 'ground':
          ...
          if cell.get('Herds') != []:
            nCarvizs = len(listHerd[cell.get('Herds')[0]].memberList)
            print('****************')
            print(f'Values of {nCarvizs} Carviz:')
            for member in listHerd[cell.get('Herds')[0]].memberList:
              print(f'Energy: {math.ceil(member.energy)}, Age: {member.age}, Social attitude: {math.ceil(member.socialAttitude)}, Lifetime: {yearLength*member.lifetime}')
          if cell.get('Prides')!= []:
            nErbasts = len(listPride[cell.get('Prides')[0]].memberList)
            print('________________')
            print(f'Values of {nErbasts} Erbast:')
            for member in listPride[cell.get('Prides')[0]].memberList:
              print(f'Energy: {math.ceil(member.energy)}, Age: {member.age}, Social attitude: {math.ceil(member.socialAttitude)}, Lifetime: {yearLength*member.lifetime}')
